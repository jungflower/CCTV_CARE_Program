...
void ScheduleRegisterPage::refreshReservationTable(const QString &customerId)
{
    ...
    ui->appointmentTable->clearContents();
    ...
    // ui->appointmentTable->sortByColumn(0, Qt::AscendingOrder);  // 시간 정렬

    for (int i = 0; i < list.size(); ++i) {
        const Reservation &r = list[i];
        ui->appointmentTable->setItem(i, 0, new QTableWidgetItem(r.date.toString("yyyy-MM-dd")));
        // ui->appointmentTable->setItem(i, 1, new QTableWidgetItem(r.time.toString("HH:mm")));
        ...
        // 체크박스 상태 변경 시 상태 텍스트도 같이 반영
        connect(checkBox, &QCheckBox::stateChanged, this, [=](int state) {
            reservationMap[selectedCustomerId][i].status =
                (state == Qt::Checked) ? ReservationStatus::Completed : ReservationStatus::Pending;

            statusItem->setText(statusToString(reservationMap[selectedCustomerId][i].status));
            statusItem->setForeground(reservationMap[selectedCustomerId][i].status == ReservationStatus::Completed
                                          ? Qt::red
                                          : Qt::black);
        });
        // connect(checkBox, &QCheckBox::stateChanged, this, [=](int state) {
        ...
        //     statusItem->setText(statusToString(r.status));
        //     statusItem->setForeground(r.status == ReservationStatus::Completed ? Qt::red : Qt::black);
        // });
